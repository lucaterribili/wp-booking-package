...
class BOOKING_PACKAGE
{
    ...
    public function __construct($shortcodes = 0, $widget = false)
    {
        ...
        if (wp_get_schedule('booking_package_notification') === false) {
            ...
            $timeStamp = mktime(date('H', $unixTime), 0, 0, date('m', $unixTime), date('d', $unixTime),
                date('Y', $unixTime));
    ...
    public function load_plugins()
    {
        ...
        if ($bool === true) {
            ...
            if ($isExtensionsValid === true) {
                ...
                $dictionary = array(
                    'You currently have a valid subscription. Do you want to deactivate the Booking Package?' => __('You currently have a valid subscription. Do you want to deactivate the Booking Package?',
                        $this->plugin_name),
                );
    ...
    public function regist_user($user_id)
    {
        ...
        $find = false;
        if (user_can($user_id,
                'subscriber') === true && intval($memberSetting['accept_subscribers_as_users']['value']) == 1) {

            $find = true;

        } else {
            if (user_can($user_id,
                    'contributor') === true && intval($memberSetting['accept_contributors_as_users']['value']) == 1) {
    ...
    public function add_pages()
    {
        ...
        if (current_user_can('manage_categories') === true) {
            ...
            if (current_user_can($manager) === true && !is_null(get_role($manager))) {

                $manager_cap = $manager;
                $editor_cap = $manager;

            } else {
                if (current_user_can($editor) === true && !is_null(get_role($editor))) {
        ...
        if ($this->siteNetwork == 1) {
            ...
            if (function_exists('get_sites') && class_exists('WP_Site_Query')) {
                ...
                if (ms_site_check() === true && is_plugin_active_for_network('booking-package/index.php') === true && intval($id) != intval(SITE_ID_CURRENT_SITE)) {
                    ...
                    add_submenu_page(__FILE__, $plugin_name, __('Subscription', $this->plugin_name), 'manage_network',
                        $this->plugin_name . '_subscription_page', array($this, 'subscription_page'));
        ...
        foreach ((array)$lists as $key => $value) {
            ...
            if (isset($calendarAccountList[$key])) {
                ...
                for ($i = 0; $i < count($value); $i++) {
                    ...
                    $this->showCustomersList($value[$i], $url, $calendarAccountList[$key]['type'], $schedule,
                        $dateFormat, $clock);
    ...
    public function showCustomersList($visitor, $url, $calendarType, $schedule, $dateFormat, $clock = '24hours')
    {
        ...
        $date = null;
        $visitor['date'] = array(
            'month' => date('n', $visitor['scheduleUnixTime']),
            'year' => date('Y', $visitor['scheduleUnixTime'])
        );
        if ($calendarType == 'day') {
            ...
            if ($clock == '12a.m.p.m') {
                ...
                $date = sprintf(__('%s:%s ' . $print_am_pm, $this->plugin_name),
                        date('h', $visitor['scheduleUnixTime']),
                        date('i', $visitor['scheduleUnixTime'])) . ' ' . $visitor['courseName'];
            ...
            if ($calendarType == 'hotel') {
                ...
                $date = sprintf(__('Until %s', $this->plugin_name),
                    $schedule->dateFormat($dateFormat, date('w', $checkOut), $checkOut, null, false, true, 'text'));
            }
        }
        ...
        <li class=''>
            ...
            <div onClick='changeStatusForDashboard(this, <?php print $visitor['key']; ?>, "<?php print $visitor['cancellationToken']; ?>", <?php print $visitor['accountKey']; ?>, "<?php print $visitor['status']; ?>", <?php print $visitor['date']['month']; ?>, 1, <?php print $visitor['date']['year']; ?>)'
                 class='status <?php print $visitor['status']; ?>'><?php print __(strtoupper($visitor['status']),
                    $this->plugin_name); ?></div>
    ...
    public function bookingPageForVisitors($atts)
    {
        ...
        if (isset($_GET['bookingID']) && isset($_GET['bookingToken'])) {
            ...
            if ($bookingDetailsResponse['status'] == 'success') {
                ...
                $verifyCancellation = $schedule->verifyCancellation($myBookingDetails, $isExtensionsValid,
                    $user['status']);
        ...
        for ($i = 0; $i < count($formData); $i++) {
            ...
            if (isset($formData[$i]['targetCustomers'])) {
                ...
                if ($target_users == 'users' && $formData[$i]['targetCustomers'] == 'users') {
                    ...
                    if ($target_users == 'visitors' && $formData[$i]['targetCustomers'] == 'visitors') {
        ...
        if (is_array($hasServices) === true && count($hasServices) != $countHasServices) {
            ...
            wp_die('<pre>' . sprintf(__('Not found services in the %s.', $pluginName),
                    $calendarAccount['name']) . '</pre>');
        ...
        if ($calendarAccount['status'] == 'close' || $calendarAccount['status'] == 'closed') {
            ...
            return '<div id="calendarStatus">' . __('We do not accept reservations for this calendar.',
                    $this->plugin_name) . '</div>';
        ...
        if ($isExtensionsValid === true) {
            ...
            if (is_string($userSubscriptions['customer_id_for_subscriptions'])) {
                ...
                $localize_script['site_subscriptions'] = substr($userSubscriptions['customer_id_for_subscriptions'],
                    -5);
        ...
        if ($calendarAccount['redirectMode'] == 'page' && !empty($calendarAccount['redirectPage'])) {
            ...
            if ($calendarAccount['redirectMode'] == 'url' && !empty($calendarAccount['redirectURL'])) {
                ...
                if ($paymentMethod[$i] == 'paypal') {
                    ...
                    if (!empty($paypal_public_key)) {
                        ...
                        wp_enqueue_script('paypal_checkout_v4_js',
                            'https://www.paypal.com/sdk/js?client-id=' . $paypal_public_key . '&currency=' . strtoupper($localize_script['currency']) . '&intent=capture&' . $localePayPal,
                            array(), null);
        ...
        if (!empty($localize_script['googleAnalytics'])) {
            ...
            wp_enqueue_script($this->prefix . 'googleAnalytics',
                'https://www.googletagmanager.com/gtag/js?id=' . $localize_script['googleAnalytics'], array(),
                $this->plugin_version);
        ...
        $html .= '			<button id="returnToDayListButton" class="hidden_panel">' . __('Return',
                $pluginName) . '</button>';
        $html .= '			<button id="returnDayButton" class="hidden_panel"></button>';
        $html .= '			<button id="nextDayButtton" class="hidden_panel"></button>';
        $html .= '			<button id="nextButton" class="right_button hidden_panel">' . __('Next',
                $pluginName) . '</button>';
        $html .= '		</div>';
        $html .= '	</div>';
        ...
        $html .= '			<span class="notifications">' . __('We sent a verification code to the following address.',
                $pluginName) . '</span> ';
        ...
        if ($localize_script['googleReCAPTCHA']['status'] === true) {
            ...
            if ($localize_script['googleReCAPTCHA']['v'] == 'v2') {
                ...
                wp_enqueue_script('recaptcha_v2_js', 'https://www.google.com/recaptcha/api.js?render=explicit', array(),
                    null);

            } else {
                if ($localize_script['googleReCAPTCHA']['v'] == 'v3') {

                    #$html .= '<script src="https://www.google.com/recaptcha/api.js?render=' . $localize_script['googleReCAPTCHA']['key'] . '" asyn defer></script>';
                    wp_enqueue_script('recaptcha_v3_js',
                        'https://www.google.com/recaptcha/api.js?render=' . $localize_script['googleReCAPTCHA']['key'],
                        array(), null);
    ...
    public function add_footer_scripts()
    {
        ...
        if ($this->loaded_plugin === true && !is_admin()) {
            ...
            errorContent.textContent = '<?php print __('Shortcode of the Booking Package cannot insert multiple shortcodes on one page.',
                $pluginName); ?>';
            var errorID = document.createElement('p');
            errorID.textContent = String('<?php print __('ID: %s has canceled the display.',
                $pluginName); ?>').replace(/%s/g, error_booking_Package_id);
        ...
        <div id="booking_pacage_users" class="wrap">
            ...
            <div id="member_list">
                ...
                <div class="page_action_panel">
                    ...
                    <button id="before_page" class="material-icons page_button w3tc-button-save button-primary">
                        navigate_before
                    </button>
                    <button id="next_page" class="material-icons page_button w3tc-button-save button-primary">
                        navigate_next
                    </button>
                        ...
                        <div class="media_toolbar">
                            <div id="buttonPanel" class="media_toolbar_primary" style="float: initial;">
                                ...
                                <div id="leftButtonPanel">
                                    <button id="beforButton"
                                            class="material-icons button media-button button-primary button-large media-button-insert">
                                        navigate_before
                                    </button>
    ...
    public function schedule_page()
    {
        ...
        wp_enqueue_script('Calendar_js', plugin_dir_url(__FILE__) . 'js/Calendar.js' . $p_v, array(),
            $this->plugin_version);
        wp_enqueue_script('input_js', plugin_dir_url(__FILE__) . 'js/Input.js' . $p_v, array(), $this->plugin_version);
        wp_enqueue_script('schedule_pange', plugin_dir_url(__FILE__) . 'js/Schedule.js' . $p_v, array(),
            $this->plugin_version);
        ...
        <div id="booking_package_calendar_accounts" style='display: none;'>
                    ...
                    <div style="overflow-x: auto;">
                        <div class="menuList">
                            ...
                            <div id="optionsForHotelLink" class="menuItem hidden_panel"><?php _e('Options',
                                    $pluginName); ?></div>
                            <div id="couponsLink" class="menuItem hidden_panel"><?php _e('Coupons',
                                    $pluginName); ?></div>
                            ...
                            <div>
                                ...
                                <div class="content">
                                    ...
                                    <div id="edit_visitor_message">
                                        ...
                                        <textarea name="emailContent" id="emailContent" class="message_body"
                                    ...
                                    <div id="edit_administrator_message" class="hidden_panel">
                                        <input type="text" id="subjectForAdmin" class="mail_subject"
                                               placeholder="Subject">
                                        <textarea name="emailContent" id="emailContentForAdmin" class="message_body"
                                                  placeholder="Message body"></textarea>
                                    </div>
                            ...
                            <tr>
                                ...
                                <td>
                                    <select id="interval_min_on_time" data-interval="5">
                                        ...
                                        for ($i = 1; $i <= 120; $i += 1) {
                                            echo '<option value="' . $i . '">' . sprintf(__('%s minutes',
                                                    $this->plugin_name), $i) . '</option>';
                                        }
                            ...
                            <tr>
                                ...
                                <td>
                                    <select id="load_deadline_time_on_time">
                                        ...
                                        for ($i = 0; $i <= BOOKING_PACKAGE_MAX_DEADLINE_TIME; $i += 30) {
                                            echo '<option value="' . $i . '">' . sprintf(__('%s min ago',
                                                    $this->plugin_name), $i) . '</option>';
                                        }
                    ...
                    <div class="inputPanel" style="border-width: 0; margin-bottom: 0;">
                        <div id="selectionSchedule_hours" class="selectBlock">
                            ...
                            <div data-key="hours" class="selectPanel closed">
                                ...
                                for ($i = 0; $i < 24; $i++) {
                                    echo '<span class="selectItem" data-key="hours" data-value="' . $i . '">' . sprintf('%02d',
                                            $i) . '</span>';
                                }
                        ...
                        <div id="selectionSchedule_minutes" class="selectBlock">
                            ...
                            <div data-key="minutes" class="selectPanel closed">
                                ...
                                for ($i = 0; $i < 60; $i++) {
                                    echo '<span class="selectItem" data-key="minutes" data-value="' . $i . '">' . sprintf('%02d',
                                            $i) . '</span>';
                                }
                        ...
                        <div id="selectionSchedule_deadline" class="selectBlock">
                            ...
                            <div data-key="deadline" class="selectPanel closed">
                                ...
                                for ($i = 0; $i <= BOOKING_PACKAGE_MAX_DEADLINE_TIME; $i += 30) {
                                    echo '<span class="selectItem" data-key="deadline" data-value="' . $i . '">' . sprintf('%02d',
                                            $i) . '</span>';
                                }
                        ...
                        <div id="selectionSchedule_capacitys" class="selectBlock">
                            ...
                            <div data-key="capacitys" class="selectPanel closed">
                                ...
                                for ($i = 0; $i <= 300; $i++) {
                                    echo '<span class="selectItem" data-key="capacitys" data-value="' . $i . '">' . sprintf('%02d',
                                            $i) . '</span>';
                                }
                        ...
                        <div id="selectionSchedule_remainders" class="selectBlock">
                            ...
                            <div data-key="remainders" class="selectPanel closed">
                                ...
                                for ($i = 0; $i <= 300; $i++) {
                                    echo '<span class="selectItem" data-key="remainders" data-value="' . $i . '">' . sprintf('%02d',
                                            $i) . '</span>';
                                }
            ...
            <div id="copyAndPasteOnCalendarSetting" class="hidden_panel">
                <a href="https://booking-package.saasproject.net/how-does-the-booking-calendar-show-on-the-page/"
                   target="_blank">
    ...
    public function setting_page()
    {
        ...
        foreach ((array)$subscriptions as $key => $value) {
            ...
            if ($key == 'expiration_date_for_subscriptions' && strlen($value) > 0) {
                ...
                $localize_script['expiration_date'] = $schedule->dateFormat($dateFormat, $positionOfWeek, $value, '',
                    true, false, 'text');
        ...
        wp_enqueue_script('Calendar_js', plugin_dir_url(__FILE__) . 'js/Calendar.js' . $p_v, array(),
            $this->plugin_version);
        wp_enqueue_script('setting_page', plugin_dir_url(__FILE__) . 'js/setting.js' . $p_v, array(),
            $this->plugin_version);
        ...
        wp_enqueue_style('codemirror_css', 'https://codemirror.net/5/lib/codemirror.css', array(),
            $this->plugin_version);
        wp_enqueue_script('codemirror_js', 'https://codemirror.net/5/lib/codemirror.js', array(),
            $this->plugin_version);
        wp_enqueue_script('codemirror_css_js', 'https://codemirror.net/5/mode/css/css.js', array(),
            $this->plugin_version);
        wp_enqueue_script('codemirror_javascript_js', 'https://codemirror.net/5/mode/javascript/javascript.js', array(),
            $this->plugin_version);
        wp_enqueue_script('jquery');
        wp_enqueue_script('wp-color-picker');
        wp_enqueue_script('jquery-ui-sortable');
            ...
            <div id="tabFrame">
                ...
                <div id="contentPanel" class="content">
                    ...
                    <div id="javascriptPanel" class="hidden_panel">
                        ...
                        ?>
                        <textarea id="javascript_booking_package"
    ...
    public function subscription_page()
    {
        ...
        <div id="subscription_page" class="wrap">
            ...
            <div style="padding: 10px 0;">
                <!--
						<button id="addSubscription" class="media-button button-primary button-large media-button-insert hidden_panel" value=""><?php _e('Add a new subscription',
                    $pluginName); ?></button>
						-->
                <button id="cancelSubscription"
                        class="media-button button-primary button-large media-button-insert hidden_panel"
                        value=""><?php _e('Cancel this subscription', $pluginName); ?></button>
                <button id="sendSubscription"
                        class="media-button button-primary button-large media-button-insert hidden_panel"
                        value=""><?php _e('Payment a new subscription', $pluginName); ?></button>
            </div>
    ...
    public function wp_ajax_booking_package_for_public()
    {
        ...
        if (isset($_POST['nonce']) && check_ajax_referer($this->action_public . '_ajax', 'nonce', false)) {
            ...
            if ($_POST['mode'] == 'user_login_for_frontend') {
                ...
                $response = $schedule->user_login_for_frontend($_POST['user_login'], $_POST['user_password'],
                    $_POST['remember']);
    ...
    public function selectedMode()
    {
        ...
        if ($_POST['mode'] == 'getRegularHolidays') {
            ...
            $startOfWeek = get_option('start_of_week', 0);
            $response = $schedule->getRegularHolidays($_POST['month'], $_POST['year'], $_POST['accountKey'],
                $startOfWeek, false);
        ...
        if ($_POST['mode'] == 'updataEmailMessage') {
            ...
            $response = $setting->updataEmailMessage($_POST['key'], $_POST['subject'], $_POST['content'],
                $_POST['enable'], $_POST['format']);
        ...
        if ($_POST['mode'] == 'deleteBookingData') {
            ...
            if ($bookingDetailsOnVisitor['status'] == 'error') {
                ...
                $response = $schedule->deleteBookingData($_POST['key'], $myBookingDetails['accountKey'], false, true,
                    $_POST['sendEmail']);
                $response['guestsList'] = $setting->getGuestsList($_POST['accountKey'], true);
                $response['account'] = $schedule->getCalendarAccount($_POST['accountKey']);
        ...
        if ($_POST['mode'] == 'lookingForSubscription') {
            ...
            $response = $setting->lookingForSubscription($_POST['customer_id_for_subscriptions'],
    ...
    public function help_calendar_box()
    {
        ...
        if ($this->locale == 'ja') {
            ...
            $content = '<ul>';
            $content .= '<li><a href="https://manual-ja.saasproject.net/%e3%83%95%e3%83%ad%e3%83%b3%e3%83%88%e3%83%9a%e3%83%bc%e3%82%b8%e3%81%ab%e4%ba%88%e7%b4%84%e3%82%ab%e3%83%ac%e3%83%b3%e3%83%80%e3%83%bc%e3%82%92%e8%a1%a8%e7%a4%ba/" target="_blank">' . __('How do I show the booking calendar on the page?',
                    $pluginName) . '</a></li>';
            $content .= '<li><a href="https://manual-ja.saasproject.net/%e3%83%97%e3%83%a9%e3%82%b0%e3%82%a4%e3%83%b3%e3%81%8b%e3%82%89%e9%80%81%e4%bf%a1%e3%81%95%e3%82%8c%e3%82%8b%e3%83%a1%e3%83%bc%e3%83%ab%e3%81%ab%e3%81%a4%e3%81%84%e3%81%a6/" target="_blank">' . __('How do I send a booking email?',
                    $pluginName) . '</a></li>';
            $content .= '<li><a href="https://manual-ja.saasproject.net/%e4%ba%88%e7%b4%84%e3%82%b9%e3%82%b1%e3%82%b8%e3%83%a5%e3%83%bc%e3%83%ab%e3%81%ae%e4%bd%9c%e6%88%90/" target="_blank">' . __('How do I create booking schedules?',
                    $pluginName) . '</a></li>';
            $content .= '</ul>';
        ...
        $content = '<ul>';
        $content .= '<li><a href="https://booking-package.saasproject.net/how-does-the-booking-calendar-show-on-the-page/" target="_blank">' . __('How do I show the booking calendar on the page?',
                $pluginName) . '</a></li>';
        $content .= '<li><a href="https://booking-package.saasproject.net/how-do-i-send-a-booking-email-with-a-plugin/" target="_blank">' . __('How do I send a booking email?',
                $pluginName) . '</a></li>';
        $content .= '<li><a href="https://booking-package.saasproject.net/how-do-i-create-booking-schedules/" target="_blank">' . __('How do I create booking schedules?',
                $pluginName) . '</a></li>';
        $content .= '</ul>';
        $screen->add_help_tab(array(
