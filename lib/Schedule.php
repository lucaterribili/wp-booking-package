...
class booking_package_schedule
{
    ...
    public function sendMail($user_email, $subject, $body, $emailFormat = 'text', $accountKey = null)
    {
        ...
        $from = $this->emailFormat(get_option($this->prefix . 'email_from', null),
            get_option($this->prefix . 'email_title_from', null));
    ...
    public function setActivationUser($user_activation_key, $user_login, $activation = 0)
    {
        ...
        if (isset($user->ID)) {
            ...
            return array(
                'status' => 'error',
                'mode' => 'notFound',
                'message' => __('Your information could not be found.', $this->pluginName)
            );
        ...
        if (is_null($row)) {
            ...
            if (intval($row['status']) == 0) {
                ...
                return array(
                    'status' => 'error',
                    'mode' => 'approved',
                    'message' => __('You have already been approved.', $this->pluginName)
                );
    ...
    public function updateUser($administrator, $accountKey)
    {
        ...
        if (intval($userId) == 0) {
            ...
            if (is_wp_error($user)) {
                ...
                do_action('booking_package_updated_user',
                    array('user_id' => $userId, 'user_login' => $_POST['user_login']));
    ...
    public function get_users($authority, $offset, $number = null, $search = null)
    {
        ...
        if (!isset($_POST['keywords'])) {
            ...
            $sql = $wpdb->prepare(
                'SELECT `key` AS `ID`, `user_login`, `email` AS `user_email`, `status`, `subscription_list`, `user_registered` FROM `' . $table_name . '` WHERE ' . implode(' OR ',
                    $queryList) . ' LIMIT %d, %d;',
                $valueList
            );
            ...
            if (isset($_POST['meta']) && intval($_POST['meta']) == 1) {
                ...
                $sql = $wpdb->prepare(
                    'SELECT `key` AS `ID`, `user_login`, `email` AS `user_email`, `status`, `subscription_list`, `user_registered`, `value` FROM `' . $table_name . '` WHERE ' . implode(' OR ',
                        $queryList) . ' LIMIT %d, %d;',
                    $valueList
                );
        ...
        if (is_null($userId)) {
            ...
            if ($userId != 0) {
                ...
                if (current_user_can($roleName) === true) {
                    ...
                    if (current_user_can('subscriber') === true && intval($memberSetting['accept_subscribers_as_users'] == 1)) {

                        $bool = true;
                        $this->find_users($userId, 1, true);

                    } else {
                        if (current_user_can('contributor') === true && intval($memberSetting['accept_contributors_as_users'] == 1)) {
                ...
                if ($bool === true) {
                    ...
                    if (!is_int($value) && is_array(array_values($value))) {
                        ...
                        $response = array(
                            'status' => 0,
                            'user' => array_merge($memberSetting, array('status' => 0, 'message' => '', 'reload' => 1))
                        );
            ...
            if (!is_int($value) && is_array($value)) {
                ...
                $response = array(
                    'status' => 0,
                    'user' => array_merge($memberSetting, array(
                        'status' => 0,
                        'message' => __('Your email address has not been accepted.', $pluginName),
                        'reload' => 1
                    ))
                );
    ...
    public function find_users($userId, $activation, $create = false)
    {
        ...
        $status = true;
        $table_name = $wpdb->prefix . 'booking_package_users';
        $sql = $wpdb->prepare('SELECT `value`,`status` FROM `' . $table_name . '` WHERE `key` = %d;',
            array(intval($userId)));
        $row = $wpdb->get_row($sql, ARRAY_A);
        if (is_null($row)) {
        ...
        if (is_null($subscription_list)) {
            ...
            foreach ((array)$subscription_list as $key => $value) {
                ...
                if ($delete === false) {
                    ...
                    $subscription_list[$key]['period_start_date'] = $this->dateFormat($dateFormat, $positionOfWeek,
                        $value['period_start'], '', true, true, 'text');
                    $subscription_list[$key]['period_end_date'] = $this->dateFormat($dateFormat, $positionOfWeek,
                        $value['period_end'], '', true, true, 'text');
    ...
    public function update_subscription($subscription)
    {
        ...
        if ($subscription['payType'] == 'stripe') {
            ...
            $add_subscription = $this->prepare_subscription($subscription['payType'], $update_subscription,
                $subscription);
    ...
    public function prepare_subscription($payType, $response_subscription, $subscription)
    {
        ...
        if ($response_subscription['status'] == 'active') {
            ...
            if ($response_subscription['status'] == 'canceled') {
                    ...
                    if ($dateFormat == 3) {
                        ...
                        if ($dateFormat == 4) {
                            ...
                            if ($dateFormat == 5) {
                                ...
                                $date = date('d', $unixTime) . ' ' . $monthList[date('n', $unixTime) - 1] . date(', Y',
                                        $unixTime);
                                    ...
                                    if ($dateFormat == 7) {
                                        ...
                                        if ($dateFormat == 8 || $dateFormat == 9) {
                                            ...
                                            if ($dateFormat == 10) {
                                                ...
                                                $date = date('d', $unixTime) . '.' . $monthList[date('n',
                                                        $unixTime) - 1] . date('.Y', $unixTime);

                                            } else {
                                                if ($dateFormat == 11) {

                                                    $date = $monthList[date('n', $unixTime) - 1] . ' ' . date('d',
                                                            $unixTime) . date(' Y', $unixTime);
        ...
        if (intval($user['status']) == 1) {
            ...
            if (isset($_POST['payType']) && $_POST['payType'] == 'stripe') {
                ...
                if (is_array($subscription)) {
                    ...
                    $stripe = $creditCard->createCustomer($_POST['payType'], $public_key, $secret_key,
                        $_POST['payToken'], $calendarAccount, $subscription, $user['user'], $payment_live,
                        $payment_active);
                    ...
                    if (isset($stripe['subscription']['status']) && $stripe['subscription']['status'] == 'active') {
                        ...
                        $add_subscription = $this->prepare_subscription($_POST['payType'], $response_subscription,
                            $subscription);
                        ...
                        $update = $this->update_subscription_list_of_user($user['user']['current_member_id'],
                            $subscription_list);
                ...
                if (isset($_POST['payType']) && $_POST['payType'] == 'paypal') {
    ...
    public function updateRegularHolidays()
    {
        ...
        return $this->getRegularHolidays($_POST['month_calendar'], $_POST['year_calendar'], $_POST['accountKey'],
    ...
    public function getCalendarList($month, $day, $year, $startOfWeek = 0)
    {
        ...
        if (intval($week_start_num) != $startOfWeek) {
            ...
            for ($i = $endDay; $i > 0; $i--) {
                ...
                if (date('w',
                        mktime(0, 0, 0, date('n', $lastUnixTime), $i, date('Y', $lastUnixTime))) == $startOfWeek) {
        ...
        $calendarList[intval($year . sprintf('%02d', $month))] = array(
            'startDay' => 1,
            'lastDay' => $last_day,
            'startWeek' => $week_start_num,
            'lastWeek' => $week_last_num,
        ...
        foreach ((array)$schedules as $key => $value) {
            ...
            if ($updateNationalHoliday === true) {
                ...
                if (isset($value['status']) && intval($value['status']) == 1) {
                    ...
                    $dayBeforeKey = date('Y', $dayBeforeUnixTime) . date('m', $dayBeforeUnixTime) . date('d',
                            $dayBeforeUnixTime);
        ...
        try {
            ...
            foreach ((array)$rows as $row) {
                ...
                for ($i = 0; $i < $maxAccountScheduleDay; $i++) {
                    ...
                    $dayBeforeNationalHolidayKey = date('Y', $dayBeforeUnixTime) . date('m',
                            $dayBeforeUnixTime) . date('d', $dayBeforeUnixTime);
                    ...
                    if (is_null($row)) {
                        ...
                        if ($calendarAccount['type'] == 'day') {
                            ...
                            $template_rows = $wpdb->get_results($wpdb->prepare($sql,
                                array(intval($accountKey), intval($week))), ARRAY_A);
                            foreach ((array)$template_rows as $template_row) {

                                $time = date('U',
                                    mktime($template_row['hour'], $template_row['min'], 0, $month, $day, $year));
                                $table_name = $wpdb->prefix . 'booking_package_schedule';

                                $this->insertSchedule(
                                    $table_name, $accountKey, $time, $month, $day, $year, $week,
                                    $template_row['hour'], $template_row['min'], $template_row['deadlineTime'],
                                    $template_row['title'],
                            ...
                            if (isset($nationalHolidays[intval($nationalHolidayKey)]) && intval($calendarAccount['hotelChargeOnNationalHoliday']) > 0) {
                                ...
                                if (isset($nationalHolidays[intval($dayBeforeNationalHolidayKey)]) && intval($calendarAccount['hotelChargeOnDayBeforeNationalHoliday']) > 0) {
    ...
    public function createCloneCalendar()
    {
        ...
        $sql = $wpdb->prepare('CREATE TEMPORARY TABLE ' . $tmp_table_name . ' SELECT * FROM ' . $table_name . ' WHERE `key` = %d;',
            array(intval($_POST['accountKey'])));
        $wpdb->query($sql);
        ...
        foreach ((array)$targetList as $key => $table) {
            ...
            if (isset($_POST[$key]) && intval($_POST[$key]) == 1) {
                ...
                $tmp_table_name = $table_name . '_tmp';
                $sql = $wpdb->prepare('CREATE TEMPORARY TABLE ' . $tmp_table_name . ' SELECT * FROM ' . $table_name . ' WHERE `accountKey` = %d;',
                    array(intval($_POST['accountKey'])));
                $wpdb->query($sql);
                $wpdb->query('ALTER TABLE ' . $tmp_table_name . ' drop `key`;');
    ...
    public function getIcalToken($accountKey)
    {
        ...
        return array(
            'status' => 'success',
            'ical' => $calendarAccount['ical'],
            'syncPastCustomersForIcal' => $calendarAccount['syncPastCustomersForIcal'],
            'icalToken' => $calendarAccount['icalToken'],
            'home' => get_home_url()
        );
    ...
    public function updateUnixTimeOnBookingData($accountKey = null, $timezone = null)
    {
        ...
        if (date_default_timezone_set($timezone)) {
            ...
            global $wpdb;
            $table_name = $wpdb->prefix . 'booking_package_schedule';
            $sql = $wpdb->prepare('SELECT * FROM `' . $table_name . "` WHERE `accountKey` = %d AND `status` = 'open';",
                array($accountKey));
            $rows = $wpdb->get_results($sql, ARRAY_A);
            foreach ((array)$rows as $row) {
    ...
    public function deletePublishedSchedules($accountKey = 1, $type = 'day')
    {
        ...
        if (isset($_POST['deletePublishedSchedules_from_month']) && isset($_POST['deletePublishedSchedules_from_day']) && isset($_POST['deletePublishedSchedules_from_year'])) {
            ...
            if (
                checkdate($_POST['deletePublishedSchedules_from_month'], $_POST['deletePublishedSchedules_from_day'],
                    $_POST['deletePublishedSchedules_from_year']) === false ||
                checkdate($_POST['deletePublishedSchedules_to_month'], $_POST['deletePublishedSchedules_to_day'],
                    $_POST['deletePublishedSchedules_to_year']) === false
            ) {
            ...
            $unixTime_from = date('U', mktime(0, 0, 0, $_POST['deletePublishedSchedules_from_month'],
                $_POST['deletePublishedSchedules_from_day'], $_POST['deletePublishedSchedules_from_year']));
            $unixTime_to = date('U', mktime(23, 59, 0, $_POST['deletePublishedSchedules_to_month'],
                $_POST['deletePublishedSchedules_to_day'], $_POST['deletePublishedSchedules_to_year']));
            global $wpdb;
            ...
            if ($_POST['delete_action'] == 'delete') {
                ...
                if ($_POST['deletionType'] == 'perfect') {
                    ...
                    if ($_POST['deletionType'] == 'incomplete') {
    ...
    public function updateStatus($bookedKey, $bookedToken, $status = 'pending')
    {
        ...
        if (is_null($row) === false) {
            ...
            $servicesDetails = $this->getSelectedServices($calendarAccount, json_decode($row['options'], true),
                $responseGuests['guests'], 'options', $coupon, $applicantCount);
        ...
        if (intval($sendEmail) == 1) {
            ...
            $email = $this->createEmailMessage($accountKey, array($email_id), $form, $accommodationDetails, $options,
                intval($bookedKey), intval($unixTimeStart), intval($timestampForUnixTime), $cancellationUri, $currency,
                $services, $payName, $payId, $scheduleTitle, $responseGuests, $coupon);
    ...
    public function getBookingDetailsOnVisitor($key, $token)
    {
        ...
        if (is_null($row) === false) {
            ...
            $row['accommodationDetailsList'] = $this->bookingDetailsForHotel($row['accountKey'],
                json_decode($row['accommodationDetails'], true), $row['currency'], 'object');
            $guests = $row['guests'];
    ...
    public function bookingDetailsForHotel($accountKey, $accommodationDetails, $currency, $mode = 'array')
    {
        ...
        $lengthOfStay = $accommodationDetails['nights'] . ' ' . $nights . ' (' . $this->formatCost($accommodationDetails['accommodationFee'],
                $currency) . ')';
        ...
        $detailsList = array(
            __('Total length of stay',
                $this->pluginName) . ': ' . $formatNightDay . ' ' . $this->formatCost($accommodationDetails['accommodationFee'],
                $currency) . ', ' . $accommodationDetails['applicantCount'] . ' ' . $roomStr
        );
        ...
        $objectList = array(
            'totalLengthOfStay' => array(
                ...
                'main' => $formatNightDay . ' ' . $this->formatCost(($accommodationDetails['accommodationFee']),
                        $currency),
                'sub' => array(),
            ),
        ...
        foreach ((array)$scheduleDetails as $key => $value) {
            ...
            $no++;
            $details = '#' . $no . ' ' . $this->dateFormat($dateFormat, $positionOfWeek, $value['unixTime'], null,
                    false, false, 'text') . ' ';
            ...
            if ($multipleRooms === true) {
                ...
                $details .= ' (' . $this->formatCost($value['cost'],
                        $currency) . ' * ' . $applicantCount . ' ' . $roomStr . ')';
                        ...
                        if (isset($room['personAmount']) && intval($room['personAmount']) > 0) {
                            ...
                            foreach ((array)$numberKeys as $nmuberKey => $numberValue) {
                                ...
                                if (intval($guests[$i][$nmuberKey]) == 0 && ($nmuberKey == 'priceOnDayBeforeNationalHoliday' || $nmuberKey == 'priceOnNationalHoliday')) {
                                    ...
                                    $guestPrice += $changePriceForGuest($scheduleDetails, array_keys($numberKeys),
                                        $nmuberKey, $guests[$i]);
                                                        ...
                                                        if (strtoupper($currency) == 'AUD') {
                                                            ...
                                                            if (strtoupper($currency) == 'GBP') {
                                                                ...
                                                                if (strtoupper($currency) == 'PHP') {
                                                                    ...
                                                                    if (strtoupper($currency) == 'CHF') {

                                                                        $cost = 'CHF ' . number_format(($cost / 100),
                                                                                2);

                                                                    } else {
                                                                        if (strtoupper($currency) == 'CZK') {

                                                                            $cost = 'Kč' . number_format(($cost / 100),
                                                                                    2);

                                                                        } else {
                                                                            if (strtoupper($currency) == 'RUB') {

                                                                                $cost = number_format(($cost / 100),
                                                                                        2) . '₽';

                                                                            } else {
                                                                                if (strtoupper($currency) == 'NZD') {

                                                                                    $cost = 'NZ\$' . number_format(($cost / 100),
                                                                                            2);

                                                                                } else {
                                                                                    if (strtoupper($currency) == 'HRK') {
    ...
    )
    {
        ...
        if ($deleteKey !== false) {
            ...
            if (is_null($row) === false) {
                ...
                #$responseGuests = json_decode($row['guests'], true);
                $responseGuests = $this->jsonDecodeForGuests($row['guests']);
                $selectedOptionsObject = $this->getSelectedOptions($calendarAccount, $row['options'],
                ...
                if ($status != 'canceled') {
                    ...
                    if ($calendarAccount['type'] == 'hotel') {
                        ...
                        #$responseGuests = json_decode($row['guests'], true);
                        $responseGuests = $this->jsonDecodeForGuests($row['guests']);
                        $servicesDetails = $this->getSelectedServices($calendarAccount,
                            json_decode($row['options'], true), $responseGuests['guests'], 'options', $coupon,
                            $applicantCount);
                        $services = $servicesDetails['object'];
                ...
                if (isset($_POST['refound']) && intval($_POST['refound']) == 1) {
                    ...
                    if (intval($payment_active) == 1 && !is_null($stripe_secret_key)) {
                        ...
                        $creditCard = new booking_package_CreditCard($this->pluginName, $this->prefix);
                        $refound = $creditCard->cancel($payId, $stripe_public_key, $stripe_secret_key, $payment_live,
                            $payToken);
                ...
                if (intval($sendEmail) == 1) {
                    ...
                    $email = $this->createEmailMessage($accountKey, array('mail_deleted'), $form, $accommodationDetails,
                        $options, $deleteKey, intval($unixTimeStart), intval($timestampForUnixTime), null, $currency,
                        $services, $payName, $payId, $scheduleTitle, $responseGuests, $coupon);
                ...
                if ($sendGC === true) {
                    ...
                    $setting = new booking_package_setting($this->prefix, $this->pluginName);
                    $ressponse['delete'] = $setting->deleteGC($accountKey, $iCalIDforGoogleCalendar,
    ...
    public function getCostsInService($calendarAccount, $service, $guests)
    {
        ...
        $costsWithKey = array(
            'cost_1' => intval($service['cost_1']),
            'cost_2' => intval($service['cost_2']),
            'cost_3' => intval($service['cost_3']),
            'cost_4' => intval($service['cost_4']),
            'cost_5' => intval($service['cost_5']),
            'cost_6' => intval($service['cost_6'])
        );
        $response = array(
        ...
        if (intval($calendarAccount['guestsBool']) == 1 && is_array($guests)) {
            ...
            foreach ($guests as $key => $guest) {
                ...
                if ($number > 0 && intval($costsWithKey[$costKey]) != 0 && intval($guest['reflectService']) == 1) {
                    ...
                    $guests[$key]['content'] = $guest['name'] . ': ' . $this->formatCost($costsWithKey[$costKey],
                            $currency) . ' * ' . $option['name'];
                    $totalCost1 += $costsWithKey[$costKey] * $number;

                } else {
                    if ($number > 0 && intval($costsWithKey[$costKey]) != 0 && intval($guest['reflectService']) == 0) {

                        $guests[$key]['content'] = $guest['name'] . ': ' . /**$this->formatCost($costsWithKey[$costKey], $currency) . ' * ' .**/
                            $option['name'];
                        if ($totalCost2 == 0) {
                        ...
                        if (isset($jsonList[$i][$key])) {
                            ...
                            if ($value == 'object') {
                                ...
                                if ($key == $targetOptions) {
                                    ...
                                    $optionsDetails = $this->getSelectedOptions($calendarAccount, $jsonList[$i][$key],
                                        $guests, $applicantCount);
                    ...
                    foreach ((array)$rows as $row) {
                        ...
                        if ($row['stop'] == 'false' && $remainder >= 0) {
                            ...
                            array_push($updateArray, array(
                                ...
                                'waitingRemainder' => intval($waitingRemainder),
                                'key' => intval($row['key'])
                            ));
                            ...
                            $error = array(
                                'status' => 'error',
                                'error' => '9503',
                                'message' => __('There is a problem with the remaining capacity in the schedules.',
                                    $this->pluginName)
                            );
    ...
    )
    {
        ...
        $customerDetailsUrl = admin_url('admin.php?page=booking-package%2Findex.php&key=' . $bookingID . '&calendar=' . $accountKey . '&month=' . date('n',
                $unixTime) . '&day=' . date('j', $unixTime) . '&year=' . date('Y', $unixTime));
        ...
        foreach ((array)$sendEamilList as $emailKey => $target) {
            ...
            foreach ((array)$emailData as $emailDataKey => $emailDataValue) {
                ...
                if ($positionTimeDate == 'dateTime') {
                    ...
                    $emailDataValue = str_replace('[bookingDateAndTime]',
                        $date['date'] . ', ' . $date['time'] . ' ' . $date['title'], $emailDataValue);
                    ...
                    $emailDataValue = str_replace('[bookingDateAndTime]',
                        $date['time'] . $date['title'] . ', ' . $date['date'], $emailDataValue);

                }


                $timestamp = $this->dateFormat($dateFormat, $positionOfWeek, $timestampForUnixTime, '', true, false,
                    'text');
                /**
                ...
                if ($calendarAccount['type'] == 'hotel') {
                    ...
                    $checkInDate = $this->dateFormat($dateFormat, $positionOfWeek, $accommodationDetails['checkIn'],
                        $scheduleTitle, false, false, 'text');
                     ...
                     * }
                     **/
                    $checkOutDate = $this->dateFormat($dateFormat, $positionOfWeek, $accommodationDetails['checkOut'],
                        $scheduleTitle, false, false, 'text');
                    $emailDataValue = str_replace('[checkOut]', $checkOutDate, $emailDataValue);

                    $detailsList = $this->bookingDetailsForHotel($accountKey, $accommodationDetails, $currency,
                        'array');
                 ...
                 * }
                 **/
                $amount = $this->getAmount($bookingID, $calendarAccount, $accommodationDetails, $services, $guests,
                    $coupon);
                $amount = $this->formatCost($amount, $currency);
                        ...
                        foreach ((array)$services as $key => $service) {
                            ...
                            if (isset($costs[0]) && is_int(intval($costs[0])) === true && intval($responseCostInService['max']) != 0) {
                                ...
                                if ($responseCostInService['hasMultipleCosts'] === true) {
                                    ...
                                    $details .= ' ' . sprintf(__('%s to %s', $this->pluginName),
                                            $this->formatCost($responseCostInService['min'], $currency),
                                            $this->formatCost($responseCostInService['max'], $currency));
                            ...
                            foreach ((array)$service['options'] as $option) {
                                ...
                                if (intval($option['selected']) == 1) {
                                    ...
                                    $no++;
                                    $details = '#' . $no . ' ' . $option['name'] . ' ';
                                    $responseCostInOption = $this->getCostsInService($calendarAccount, $option,
                                        $guestsList);
                                    $costs = $responseCostInOption['costs'];
                                    if (is_int(intval($costs[0])) === true && intval($costs[0]) != 0) {
                                        ...
                                        if ($responseCostInOption['hasMultipleCosts'] === true) {
                                            ...
                                            $details .= ' ' . sprintf(__('%s to %s', $this->pluginName),
                                                    $this->formatCost($responseCostInOption['min'], $currency),
                                                    $this->formatCost($responseCostInOption['max'], $currency));
            ...
            if (intval($twilio_active) == 1 && !empty($twilio_sid) && !empty($twilio_token)) {
                ...
                for ($i = 0; $i < count($visitorSMS); $i++) {
                    ...
                    if ($twilio_sendingMethod == 'phoneNumber' && !empty($twilio_number)) {
                        ...
                        if ($twilio_sendingMethod == 'senderID') {
                    ...
                    if ($send === true) {
                        ...
                        $response = wp_remote_request('https://api.twilio.com/2010-04-01/Accounts/' . $twilio_sid . '/Messages.json',
                            $args);
                        $statusCode = wp_remote_retrieve_response_code($response);
                        $response = json_decode(wp_remote_retrieve_body($response), true);
        ...
        if ($ical === false) {
            ...
            foreach ((array)$calendarList as $key => $value) {
                ...
                for ($i = $value['startDay']; $i <= $value['lastDay']; $i++) {
                    ...
                    if (!is_null($schedule_start_day) && intval(date('Ymd',
                            mktime(0, 0, 0, $value['month'], $i, $value['year']))) < $schedule_start_day) {
                    ...
                    if ($public == false) {
                        ...
                        if (is_null($rows) === false && count($rows) != 0) {
                            ...
                            for ($row = 0; $row < count($rows); $row++) {
                                ...
                                $response = $this->getVistorsBookedList($rows[$row], $account['type'],
                                    $reservationForHotel, $numberKeys);
    ...
    public function fixUnixTimeShift($schedule, $timezone)
    {
        ...
        $trueUnixTime = date('U',
            mktime($schedule['hour'], $schedule['min'], 0, $schedule['month'], $schedule['day'], $schedule['year']));
                ...
                if (isset($bookedServices[$dayKey])) {
                    ...
                    if (isset($bookedServices[$dayKey][$timeKey])) {
                        ...
                        if (isset($bookedServices[$dayKey][$timeKey][$service['key']])) {
                            ...
                            $bookedServices[$dayKey][$timeKey][$service['key']]['count']++;
                            array_push($bookedServices[$dayKey][$timeKey][$service['key']]['durationTimes'],
                                $durationTime);
                            if ($bookedServices[$dayKey][$timeKey][$service['key']]['maximumDurationTime'] < $durationTime) {
    ...
    public function deleteCalendarAccount()
    {
        ...
        $response = array();
        $table_name = $wpdb->prefix . 'booking_package_calendarAccount';
        $sql = $wpdb->prepare('SELECT * FROM `' . $table_name . '` WHERE `schedulesSharing` = %d AND `targetSchedules` = %d;',
            array(1, $_POST['accountKey']));
        $rows = $wpdb->get_results($sql, ARRAY_A);
        if (count($rows) == 0) {
            ...
            $response = array(
                'error' => 1,
                'message' => __('If you want to delete this calendar, delete the calendar sharing the schedules.',
                        $this->pluginName) . "\n" . $calendarName
            );
                    ...
                    if ($value->existence == 0) {
                        ...
                        if (is_null($row)) {
                            ...
                            $this->insertSchedule(
                                ...
                                $value->year, $value->weekKey, $value->hour, $value->min, $value->deadlineTime,
                                $value->title,
    ...
    public function getAccountScheduleData($getDeletedDate = false)
    {
        ...
        $scheduleData = array();
        $jsonAraay = array(
            'completeFlag' => 'accountScheduleData',
            'startDay' => 1,
            'lastDay' => intval($last_day),
            'startWeek' => intval($week_start_num),
            'lastWeek' => intval($week_last_num),
            'month' => intval($month),
            'year' => intval($year),
            'timestamp' => date('U')
        );
    ...
    public function updateAccountTemplateSchedule()
    {
        ...
        for ($i = 0; $i < $_POST['timeCount']; $i++) {
            ...
            if (is_array($row)) {
                ...
                if ($schedule['delete'] == 'true' || $schedules[sprintf('%02d',
                        intval($schedule['hour'])) . sprintf('%02d', intval($schedule['min']))]) {
            ...
            $schedules[sprintf('%02d', intval($schedule['hour'])) . sprintf('%02d',
                intval($schedule['min']))] = $schedule;
    ...
    public function updateAccountSchedule()
    {
        ...
        try {
            ...
            for ($i = 0; $i < $_POST['timeCount']; $i++) {
                ...
                #$schedule = json_decode(str_replace("\\", "", $_POST['schedule' . $i]), true);
                $schedule = json_decode(stripslashes($_POST['schedule' . $i]), true);
                $unixTime = intval(date('U',
                    mktime($schedule['hour'], $schedule['min'], 0, $_POST['month'], $_POST['day'], $_POST['year'])));
                $weekKey = intval(date('w',
                    mktime($schedule['hour'], $schedule['min'], 0, $_POST['month'], $_POST['day'], $_POST['year'])));
                ...
                if (isset($schedule['key'])) {
                    ...
                    $valueArray = array(
                        ($unixTime - ($courseTime * 60) - ($maintenanceTime * 60)),
                        $unixTime,
                        intval($accountKey)
                    );
                    #var_dump($valueArray);
        ...
        if (intval($calendarAccount['cancellationOfBooking']) == 1) {
            ...
            if ($unixTime < intval($bookingDetails['scheduleUnixTime'])) {
                ...
                if ($calendarAccount['refuseCancellationOfBooking'] == 'not_refuse') {
                    ...
                    if ($bookingDetails['status'] == $calendarAccount['refuseCancellationOfBooking']) {
    ...
    public function getDownloadCSV()
    {
        ...
        if (isset($_POST['day']) && $_POST['day'] != '') {
            ...
            $stopUnixTime = date('U',
                mktime(23, 59, 59, intval($_POST['month']), intval($lastDay), intval($_POST['year'])));
        ...
        foreach ((array)$rows as $row) {
            ...
            if ($calendarAccount['type'] == 'day') {
                ...
                $customer['scheduleDate'] = $this->dateFormat($dateFormat, $positionOfWeek, $row['scheduleUnixTime'],
                    $row['scheduleTitle'], true, false, 'text');
                ...
                $selectedOptionsObject = $this->getSelectedOptions($calendarAccount, $row['options'],
                    $responseGuests['guests']);
                $servicesDetails = $this->getSelectedServices($calendarAccount, json_decode($row['options'], true),
                    $responseGuests['guests'], 'options', $coupon, $row['applicantCount']);
                $services = $servicesDetails['object'];
                ...
                $customer['checkIn'] = $this->dateFormat($dateFormat, $positionOfWeek, $row['checkIn'], null, false,
                    false, 'text');
                $customer['checkOut'] = $this->dateFormat($dateFormat, $positionOfWeek, $row['checkOut'], null, false,
                    false, 'text');
                $accommodationDetails = json_decode($row['accommodationDetails'], true);
                $customer['adults'] = 0;
                $customer['children'] = 0;
            ...
            for ($i = 0; $i < count($praivateData); $i++) {
                ...
                if (is_string($praivateData[$i]['value'])) {
                    ...
                    if (is_array($praivateData[$i]['value'])) {
    ...
    public function sendVerificationCode($administrator = false)
    {
        ...
        $response = array(
            ...
            'bookingVerificationCode' => $bookingVerificationCode
        );
        ...
        if ($bookingVerificationCode != 'false') {
            ...
            $subject = __('Verification code', $this->pluginName) . ' [' . get_option($this->prefix . 'site_name',
                    'Booking Package') . ']';
            $body = sprintf(__('Your verification code is: %s', $this->pluginName),
                    $verificationCode) . "\n\n" . get_option($this->prefix . 'site_name',
                    'Booking Package') . "\n" . $from;
    ...
    public function get_user_id($administrator = false, $request_user_id = null)
    {
        ...
        if ($administrator === false) {
            ...
            if ($administrator === true && isset($request_user_id)) {
    ...
    private function getUserValues($accountKey, $type, $administrator, $personalInformation = null, $user_id = null)
    {
        ...
        for ($i = 0; $i < count($form); $i++) {
            ...
            if (isset($_POST['form' . $i])) {
                ...
                if (($form[$i]['required'] == 'true' || $form[$i]['required'] == 'true_frontEnd') && strlen(preg_replace('/( |　)/',
                        '', $value)) == 0) {
                    ...
                    if ($administrator === true && $form[$i]['required'] == 'true') {
                        ...
                        return array(
                            'status' => 'error',
                            'message' => stripslashes('Invalid value in the "' . $form[$i]['name'] . '".'),
                            'form' => $form[$i]
                        );
                    ...
                    if ($form[$i]['isEmail'] == 'true' && strlen($value) != 0 && is_email($value) === false) {
                        ...
                        return array(
                            'status' => 'error',
                            'message' => __('The format of the email address is incorrect.',
                                    $this->pluginName) . "\n" . $form[$i]['name'],
                            'form' => $form[$i]
                        );
    ...
    public function checkVerificationCode($administrator = false)
    {
        ...
        $response = array(
            ...
            'verificationHashCode' => wp_hash($_POST['verificationCode']),
            'error_message' => __('The verification code is incorrect.', $this->pluginName)
        );
        /**
    ...
    public function sendBooking($administrator = false)
    {
        ...
        $paymentMethod = explode(',', $calendarAccount['paymentMethod']);
        $preparation = array(
            'time' => intval($calendarAccount['preparationTime']),
            'position' => $calendarAccount['positionPreparationTime']
        );
        $response_user = $this->get_user_id($administrator, $_POST['userId']);
        ...
        if (is_null($row)) {
            ...
            $this->cancelPayment();
            $response = $this->getReservationData(intval($_POST['month']), intval($_POST['day']),
                intval($_POST['year']), false, $public);
            $response['status'] = 'error';
            ...
            if ($row['unixTime'] == $scheduleUnixTime) {
                ...
                if ($calendarAccount['type'] == 'hotel' && isset($_POST['json'])) {
                    ...
                    $accommodationDetails = $this->createAccommodationDetails($accountKey, $accountCalendarKey,
                        $_POST['json'], $sql_start_unixTime, $applicantCount, 'book', null);
                    ...
                    if (isset($_POST['courseKey']) || isset($_POST['selectedCourseList'])) {
                        ...
                        $servicesDetails = $this->getSelectedServices($calendarAccount, $_POST['selectedCourseList'],
                            $guests, 'selectedOptionsList', $coupon, $applicantCount);
                        $services = $servicesDetails['object'];
                        foreach ((array)$services as $key => $service) {
                        ...
                        if (intval($calendarAccount['blockSameTimeBookingByUser']) == 1) {
                            ...
                            $blockSameTimeBookingByUser = $this->blockSameTimeBookingByUser($response_user['user_id'],
                                $calendarAccount, $sql_start_unixTime, $sql_max_unixTime, $emails);
                        ...
                        $valueArray = array(
                            intval($accountCalendarKey),
                            intval($sql_start_unixTime),
                            intval($sql_max_unixTime)
                        );
                        ...
                        if (intval($calendarAccount['blockSameTimeBookingByUser']) == 1) {
                            ...
                            $blockSameTimeBookingByUser = $this->blockSameTimeBookingByUser($response_user['user_id'],
                                $calendarAccount, $sql_start_unixTime, null, $emails);
                            if ($blockSameTimeBookingByUser['status'] == 'error') {
                ...
                if (isset($_POST['payToken'])) {
                    ...
                    $creditCard = new booking_package_CreditCard($this->pluginName, $this->prefix);
                    $currency = get_option($this->prefix . 'currency', 'usd');
                    $amount = $this->getAmount($lastID, $calendarAccount, $accommodationDetails, $services,
                        $responseGuests, $coupon);
                    if (intval($payment_active) == 1 && !empty($secret_key)) {

                        $payResponse = $creditCard->pay($_POST['payType'], $stripe_konbini, $public_key, $secret_key,
                            $_POST['payToken'], $payment_live, $amount, $currency, $lastID, $visitorName, $visitorEmail,
                            $visitorBookingDate);
        ...
        if (intval($_POST['sendEmail']) == 1) {
            ...
            $email = $this->createEmailMessage($accountKey, array('mail_new_admin'), $form, $accommodationDetails,
                $selectedOptions, $lastID, $scheduleUnixTime, $sendDate, $cancellationUri, $currency, $services,
                $payName, $payId, $scheduleTitle, $responseGuests, $coupon);
        ...
        $ressponse = $this->getReservationData(intval($_POST['month']), intval($_POST['day']), intval($_POST['year']),
            $iCal, $public);
    ...
    public function serachCoupons($unixTime, $couponID, $accountKey)
    {
        ...
        if (!empty($coupon)) {
                ...
                if (intval($user['status']) == 1) {
                    ...
                    if ($coupon['limited'] == 'limited') {
                        ...
                        if (intval($usedCoupon['COUNT(`key`)']) > 0) {
                            ...
                            $response['message'] = sprintf(__('You have already used the coupon code of "%s".',
                                $this->pluginName), $couponID);
                            return $response;
                    ...
                    $response['message'] = sprintf(__('Not found the coupon code of "%s".', $this->pluginName),
                            $couponID) . " \nCause: 1";
                    return $response;
            ...
            if (intval($coupon['expirationDateStatus']) == 1) {
                ...
                $isBooking = $this->validExpirationDate($currentDate, $coupon['expirationDateStatus'],
                    $coupon['expirationDateFrom'], $coupon['expirationDateTo']);
    ...
    public function createAccommodationDetails(
        $originCalendarKey,
        $calendarAccountKey,
        $json,
        $sql_start_unixTime,
    ...
    )
    {
        ...
        if (count($scheduleList) != 0) {
            ...
            if (count($rows) != 0 && !is_null($rows)) {
                ...
                foreach ((array)$rows as $row) {
                    ...
                    $dateFormat = intval(get_option($this->prefix . 'dateFormat', 0));
                    $positionOfWeek = get_option($this->prefix . 'positionOfWeek', 'before');
                    $date = $this->dateFormat($dateFormat, $positionOfWeek, $row['unixTime'], $row['title'], false,
                        false, 'text');
                    if ($type == 'book' && $row['remainder'] <= 0) {

                        return array(
                            'status' => 'error',
                            'message' => sprintf(__('There is no vacancy in the room on %s'), $date)
                        );
                    ...
                    if ($this->confirmRegularHolidays($accountKey, $row['month'], $row['day'], $row['year']) === true) {
                        ...
                        return array(
                            'status' => 'error',
                            'message' => __('The schedule you requested has been closed.', $this->pluginName)
                        );
                    ...
                    if ($row['stop'] == 'true') {
                        ...
                        return array(
                            'status' => 'error',
                            'message' => sprintf(__('Booking of %s is suspended.'), $date)
                        );
                    ...
                    if ($i != 0) {
                        ...
                        if ($time < 1440) {
                            ...
                            return array(
                                'status' => 'error',
                                'message' => __('Schedule was not found', $this->pluginName),
                                'sql' => $sql
                            );
                ...
                $accommodationDetails['scheduleDetails'] = $this->addPriceKeyByDayOfWeek($accommodationDetails['scheduleDetails'],
                    $numberKeys, false);
            ...
            foreach ((array)$jsonList['rooms'] as $roomKey => $room) {
                ...
                foreach ((array)$guestsList as $key => $value) {
                    ...
                    if ($selected === false) {
                        ...
                        array_unshift($guestsArray,
                            array('number' => 0, 'price' => 0, 'name' => 'SELECT', 'selected' => 1));

                    } else {

                        array_unshift($guestsArray,
                            array('number' => 0, 'price' => 0, 'name' => 'SELECT', 'selected' => 0));
        ...
        if (count($jsonList['taxes']) != 0) {
            ...
            $taxList = array();
            $table_name = $wpdb->prefix . 'booking_package_taxes';
            $sql = $wpdb->prepare('SELECT * FROM ' . $table_name . " WHERE `accountKey` = %d AND `active` = 'true' ORDER BY ranking ASC;",
                array(intval($originCalendarKey)));
            $rows = $wpdb->get_results($sql, ARRAY_A);
                ...
                if (intval($tax['expirationDateStatus']) == 1) {
                    ...
                    if ($tax['expirationDateTrigger'] != 'dateBooked') {
                        ...
                        foreach ($accommodationDetails['scheduleDetails'] as $scheduleKey => $schedule) {
                            ...
                            $expirationDate = $schedule['year'] . sprintf('%02d%02d', $schedule['month'],
                                    $schedule['day']);
                            $isTax = $this->validExpirationDate(intval($expirationDate),
                                intval($tax['expirationDateStatus']), intval($tax['expirationDateFrom']),
                                intval($tax['expirationDateTo']));
                            if ($isTax === false) {
                ...
                if ($tax['target'] == 'room') {
                    ...
                    if ($tax['scope'] == 'day') {
                        ...
                        if ($tax['scope'] == 'booking') {
                            ...
                            if ($tax['scope'] == 'bookingEachGuests') {
    ...
    public function createTaxesDetails($accountKey, $totalCost, $bookingYMD = 0, $applicantCount = null)
    {
        ...
        $isExtensionsValid = $this->getExtensionsValid(false);
        $table_name = $wpdb->prefix . 'booking_package_taxes';
        $sql = $wpdb->prepare('SELECT * FROM ' . $table_name . ' WHERE `accountKey` = %d AND `active` = %s ORDER BY ranking ASC;',
            array(intval($accountKey), 'true'));
        #var_dump($sql);
        $rows = $wpdb->get_results($sql, ARRAY_A);
        foreach ((array)$rows as $key => $tax) {
            ...
            if ($bookingYMD != 0 && intval($tax['expirationDateStatus']) == 1) {
                ...
                $isTax = $this->validExpirationDate($bookingYMD, intval($tax['expirationDateStatus']),
                    intval($tax['expirationDateFrom']), intval($tax['expirationDateTo']));
                if ($isTax === false) {
    ...
    public function getSelectedGuests($calendarAccount, $selectedGuestsString, $mode)
    {
        ...
        for ($guestKey = 0; $guestKey < count($selectedGuests); $guestKey++) {
            ...
            $sql = $wpdb->prepare('SELECT * FROM ' . $table_name . ' WHERE `key` = %d ORDER BY ranking ASC;',
                array(intval($selectedGuest['key'])));
            $row = $wpdb->get_row($sql, ARRAY_A);
            if (is_null($row)) {
            ...
            if ($response['reflectAdditional'] == 1) {
                ...
                if ($response['reflectAdditional'] > 1) {
                    ...
                    $response['reflectAdditionalTitle'] = $response['reflectAdditional'] . __('people',
                            $this->pluginName);
        ...
        if ($minimumGuests['enabled'] == 1 && $minimumGuests['number'] > 0) {
            ...
            if ($minimumGuests['included'] == 1 && $minimumGuests['number'] > ($response['requiredTotalNumberOfGuests'] + $response['unrequiredTotalNumberOfGuests'])) {
                ...
                $response['message'] = sprintf(__('The total number of people must be %s or more.', $this->pluginName),
                    $minimumGuests['number']);

            } else {
                if ($minimumGuests['number'] > $response['requiredTotalNumberOfGuests']) {

                    $response['isGuests'] = false;
                    $response['message'] = sprintf(__('The required total number of people must be %s or more.',
                        $this->pluginName), $minimumGuests['number']);
        ...
        if ($maximumGuests['enabled'] == 1 && $maximumGuests['number'] > 0) {
            ...
            if ($maximumGuests['included'] == 1 && $maximumGuests['number'] < ($response['requiredTotalNumberOfGuests'] + $response['unrequiredTotalNumberOfGuests'])) {
                ...
                $response['isGuests'] = false;
                $response['message'] = sprintf(__('The total number of people must be %s or less.', $this->pluginName),
                    $maximumGuests['number']);

            } else {
                if ($maximumGuests['number'] < $response['requiredTotalNumberOfGuests']) {

                    $response['isGuests'] = false;
                    $response['message'] = sprintf(__('The required total number of people must be %s or less.',
                        $this->pluginName), $maximumGuests['number']);
    ...
    public function serachCourse($accountKey, $scheduleKey, $key = false, $bookingYMD = null, $time = false)
    {
        ...
        if (is_null($row)) {
            ...
            return array(
                'status' => 'error',
                'message' => sprintf(__('%s was not found', $this->pluginName), 'Service')
            );
            ...
            $isBooking = $this->validExpirationDate(intval($bookingYMD), intval($row['expirationDateStatus']),
                intval($row['expirationDateFrom']), intval($row['expirationDateTo']));
            ...
            if ($isBooking === false) {
                ...
                return array(
                    'status' => 'error',
                    'message' => sprintf(__('%s was not found', $this->pluginName), $row['name'])
                );
            ...
            if ($invalidService === false) {
                ...
                return array(
                    'status' => 'error',
                    'message' => sprintf(__('%s was not found', $this->pluginName), $row['name']) . ' #2'
                );
    ...
    public function invalidService($accountKey, $scheduleKey, $requestService, $bookingYMD)
    {
        ...
        if ($requestService['stopServiceUnderFollowingConditions'] == 'isNotEqual' || $requestService['stopServiceUnderFollowingConditions'] == 'isEqual') {
            ...
            if ($requestService['stopServiceUnderFollowingConditions'] == 'isNotEqual') {
                ...
                if ($requestService['stopServiceUnderFollowingConditions'] == 'isEqual') {
            ...
            if ($requestService['stopServiceUnderFollowingConditions'] == 'specifiedNumberOfTimes') {
                ...
                if ($requestService['stopServiceForDayOfTimes'] == 'timeSlot') {
                    ...
                    if ($requestService['stopServiceForDayOfTimes'] == 'day') {
    ...
    public function blockSameTimeBookingByUser($user_id, $calendarAccount, $startUnix, $endUnix, $emails)
    {
        ...
        if (!is_null($user_id)) {
            ...
            if (!is_null($endUnix)) {
                ...
                $response['message'] = sprintf(__('You have already booked between %s and %s.', $this->pluginName),
                    $this->dateFormat($dateFormat, $positionOfWeek, $startUnix, '', true, true, 'text'),
                    $this->dateFormat($dateFormat, $positionOfWeek, $endUnix, '', true, true, 'text'));
                ...
                $response['message'] = sprintf(__('You have already booked at %s.', $this->pluginName),
                    $this->dateFormat($dateFormat, $positionOfWeek, $startUnix, '', true, true, 'text'));
        ...
        if (is_array($emails)) {
            ...
            for ($i = 0; $i < count($emails); $i++) {
                ...
                if (!is_null($endUnix)) {
                    ...
                    $response['message'] = sprintf(__('You have already booked between %s and %s by %s.',
                        $this->pluginName),
                        $this->dateFormat($dateFormat, $positionOfWeek, $startUnix, '', true, true, 'text'),
                        $this->dateFormat($dateFormat, $positionOfWeek, $endUnix, '', true, true, 'text'), $email);
                    ...
                    $response['message'] = sprintf(__('You have already booked at %s by %s.', $this->pluginName),
                        $this->dateFormat($dateFormat, $positionOfWeek, $startUnix, '', true, true, 'text'), $email);
    ...
    )
    {
        ...
        return array(
            'lastID' => $lastID,
            'cancellationToken' => $cancellationToken,
            'cancellationUri' => 'id=' . $lastID . '&token=' . $cancellationToken
        );
        #return $lastID;
    ...
    public function setUserInformation($form)
    {
        ...
        if ($userId != 0 && current_user_can($roleName) === true) {
            ...
            if ($userId != 0 && current_user_can('subscriber') === true && intval($memberSetting['accept_subscribers_as_users'] == 1)) {

                $bool = true;

            } else {
                if ($userId != 0 && current_user_can('contributor') === true && intval($memberSetting['accept_contributors_as_users'] == 1)) {
    ...
    public function updateBooking($administrator)
    {
        ...
        $table_name = $wpdb->prefix . 'booking_package_userPraivateData';
        $sql = 'SELECT * FROM `' . $table_name . '` WHERE `key` = %d;';
        $sql = $wpdb->prepare('SELECT * FROM `' . $table_name . '` WHERE `key` = %d;',
            array(intval($_POST['updateKey'])));
        $row = $wpdb->get_row($sql, ARRAY_A);
        if (is_null($row) === false) {
            ...
            $bookingYMD = date('Y', $row['scheduleUnixTime']) . date('m', $row['scheduleUnixTime']) . date('d',
                    $row['scheduleUnixTime']);
            ...
            if ($calendarAccount['type'] == 'hotel') {
                ...
                $accommodationDetails = json_decode($row['accommodationDetails'], true);
                $accommodationDetails = $this->createAccommodationDetails($calendarAccount['key'], $accountCalendarKey,
                    $_POST['json'], $unixTimeStart, $applicantCount, 'update', $accommodationDetails);
            ...
            if (isset($_POST['update_booking_date']) || isset($_POST['update_booking_course'])) {
                ...
                #$responseGuests = json_decode($row['guests'], true);
                $responseGuests = $this->jsonDecodeForGuests($row['guests']);
                $servicesDetails1 = $this->getSelectedServices($calendarAccount, json_decode($row['options'], true),
                    $responseGuests['guests'], 'options', $coupon, $applicantCount);
                ...
                if ($calendarAccount['type'] == 'hotel') {
                    ...
                    $servicesDetails2 = $this->getSelectedServices($calendarAccount, $_POST['options'],
                        $responseGuests['guests'], 'options', $coupon, $applicantCount);
                    ...
                    foreach ((array)$selectedServices as $service) {
                        ...
                        if (isset($rowCourse['status']) && $rowCourse['status'] == 'error') {
                            ...
                            return array(
                                'status' => 'error',
                                'error' => '9020',
                                'servicesDetails2' => $servicesDetails2,
                                'rowCourse' => $rowCourse,
                                'accountKey' => $accountKey,
                                'message' => $rowCourse['message']
                            );
    ...
    public function changeBookingTime(
        $mode,
        $updateKey,
        $updateScheduleKey,
        $status,
        ...
        foreach ((array)$rows as $row) {
            ...
            if ($row['remainder'] < 0 || $row['stop'] == 'true') {
                ...
                return array(
                    'status' => 'error',
                    'event' => 'return',
                    'message' => 'There is not enough remainder in the capacity.'
                );
                break;
            ...
            if (!is_null($rows)) {
                ...
                foreach ((array)$rows as $row) {
                    ...
                    if (!empty($row['permalink']) && !empty($row['cancellationToken'])) {
                        ...
                        $cancellationUri = $this->getCancellationUri($row['permalink'], $row['key'],
                            $row['cancellationToken']);

                    }

                    $email = $this->createEmailMessage($calendarAccount['key'], array('mail_reminder'), $form,
                        $accommodationDetails, $options, intval($row['key']), intval($row['scheduleUnixTime']),
                        intval($row['reserveTime']), $cancellationUri, $row['currency'], $services, $row['payName'],
                        $row['payId'], $row['scheduleTitle'], $responseGuests, $coupon);
