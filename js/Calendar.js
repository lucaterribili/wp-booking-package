...
Booking_App_Calendar.prototype.create = function (calendarPanel, calendarData, month, day, year, permission, callback) {
    ...
    for (let key in calendar) {
        let className = 'dayPanel dayPanelHeight';
        let dataKey = parseInt(calendar[key].year + ("0" + calendar[key].month).slice(-2) + ("0" + calendar[key].day).slice(-2));
        let bool = 1;
        ...
        var data = {
            key: dataKey,
            week: parseInt(calendar[key].week),
            month: calendar[key].month,
            day: calendar[key].day,
            year: calendar[key].year,
            eventPanel: dayPanel,
            status: true,
            count: i,
            bool: bool,
        ...
        if (calendarData.calendar[dataKey] != null || (calendarData.reservation != null && calendarData.reservation[dataKey])) {
            ...
            dayPanel.setAttribute("class", "dayPanel dayPanelHeight closeDay disabled");

            if (parseInt(weekLine) == 1) {

                dayPanel.setAttribute("class", "border_bottom_width dayPanel dayPanelHeight closeDay disabled");
...
Booking_App_Calendar.prototype.holidayPanel = function (mode, holidayPanel, calendarPanel, month, year, regularHolidays, callback) {
    ...
    object.create(calendarPanel, regularHolidays, month, 1, year, '', function (callbackOnDay) {
        ...
        callbackOnDay.eventPanel.onclick = function () {
            ...
            var postData = {
                mode: 'updateRegularHolidays',
                /**nonce: object._nonce, action: object._action,**/ accountKey: mode,
                day: holiday.day,
                month: holiday.month,
                year: holiday.year,
                month_calendar: regularHolidays.date.month,
                year_calendar: regularHolidays.date.year,
                status: 0
            };
...
Booking_App_ObjectsControl.prototype.getCostsInService = function (service, guests, isGuests, isExtensionsValid) {
    ...
    var costsWithKey = {
        cost_1: parseInt(service.cost_1),
        cost_2: parseInt(service.cost_2),
        cost_3: parseInt(service.cost_3),
        cost_4: parseInt(service.cost_4),
        cost_5: parseInt(service.cost_5),
        cost_6: parseInt(service.cost_6)
    };
    var costs = [];
            ...
            if (response.status === true) {
                ...
                sendButton.onclick = function () {
                    ...
                    if (verificationCode.length == 6 && isNaN(Number(verificationCode)) === false) {
                        ...
                        var checkVerificationCodePost = {
                            nonce: nonce,
                            action: action,
                            mode: prefix + 'checkVerificationCode',
